program Spin
    ...
    real*8, parameter  :: BETA_FINAL = 3.0d0
    real*8, parameter  :: db = 0.001d0
    integer :: mesh(L,L)
    ...
    open(unit=20, file='iterations.out', status='replace')

    b = 3.0d0
    iterations = 0

    !do while (b <= BETA_FINAL)
    do while (iterations <= 3000)
        do i = 1, L * L
        ...
        write(20, '(I5, F15.6)') iterations, E_per_spin
...
contains
    ...
    subroutine print_mesh_final(mesh)
        ...
        open(10, file='configuracao_final_iterations.out', status='replace')
        do i = 1, L
            do j = 1, L
